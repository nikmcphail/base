...
void md5::init(md5_context_t* context) {
...
void md5::update(md5_context_t* context, unsigned char const* buf, unsigned int len) {
  unsigned int t;
  ...
  if (t) {
    ...
    memcpy(p, buf, t);
    md5_transform(context->buf, (unsigned int*)context->in);
    buf += t;
    len -= t;
  }
...
void md5::final(unsigned char digest[MD5_DIGEST_LENGTH], md5_context_t* context) {
  unsigned       count;
  unsigned char* p;
...
unsigned int md5::pseudo_random(unsigned int seed) {
  ...
  init(&context);
  update(&context, (unsigned char*)&seed, sizeof(seed));
